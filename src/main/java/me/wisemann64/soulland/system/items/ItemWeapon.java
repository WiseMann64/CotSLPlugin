...
public class ItemWeapon extends ItemAbstract implements ItemModifiable {
    ...
    private void putBase(@NotNull ItemStack item) {
        ...
        modifiable.set(SLItems.key("apply"), PersistentDataType.STRING,"main");
        modifiable.set(SLItems.key("max_upgrade"),PersistentDataType.INTEGER,maxUpgrade);
        modifiable.set(SLItems.key("upgrade"),PersistentDataType.INTEGER,upgrade);
        modifiable.set(SLItems.key("slot"),PersistentDataType.INTEGER,hasSlot ? 1 : 0);
        modifiable.set(SLItems.key("power"),PersistentDataType.STRING,power == null ? "NONE" : power);
        modifiable.set(SLItems.key("subtype"),PersistentDataType.STRING,subtype.toString());

        PersistentDataContainer base = modifiable.getAdapterContext().newPersistentDataContainer();
        modifiers.keySet().forEach(k -> base.set(SLItems.key(k.getPath()),PersistentDataType.DOUBLE,modifiers.get(k)));

        modifiable.set(SLItems.key("base"),PersistentDataType.TAG_CONTAINER,base);

        PersistentDataContainer calculated = modifiable.getAdapterContext().newPersistentDataContainer();
        calculate(calculated);

        modifiable.set(SLItems.key("calculated"),PersistentDataType.TAG_CONTAINER,calculated);

        body.set(SLItems.key("modifiable"),PersistentDataType.TAG_CONTAINER,modifiable);
    ...
    private void calculate(PersistentDataContainer cont) {
        for (ItemModifiers v : modifiers.keySet()) {
            if (v == ItemModifiers.DAMAGE || v == ItemModifiers.PROJECTILE_DAMAGE || v == ItemModifiers.MAGIC_DAMAGE) {
                ...
                cont.set(SLItems.key(v.getPath()),PersistentDataType.DOUBLE,newAtk);
                calculated.put(v,newAtk);
            } else {
                cont.set(SLItems.key(v.getPath()), PersistentDataType.DOUBLE, modifiers.get(v));
